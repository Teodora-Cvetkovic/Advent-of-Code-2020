...
let rec najdi_pare a b = function
  | [] -> None
  | x :: xs -> 
    if a > (x + b) then
      match najdi_razliko a (x + b) xs with
      | None -> najdi_pare a b xs
      | Some y -> Some (x, y)
    else
      najdi_pare a b xs

let rec zmnozi_trojico = function
  | [] -> "Ni takšne trojice!"
  | x :: xs ->
    match najdi_pare 2020 x xs with
    | None -> zmnozi_trojico xs
    | Some (y, z) -> string_of_int (x * y * z)

let naloga2 data =
  let lista = String.split_on_char '\n' data in
  lista |> int_list |> zmnozi_trojico

let resitev () =
  print_endline "Solving Day: 1";
  let input_data = preberi_datoteko "day_1.in" in
  let odgovor1 = naloga1 input_data in
  let odgovor2 = naloga2 input_data in
